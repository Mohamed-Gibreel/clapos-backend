...
ClaPos is a multi-tenant SaaS POS. The base-template's two-level isolation is used directly, with the "Project" concept renamed to "Store" and a third level (POS Terminal) added on top.
...
SuperAdmin (platform-level)
  ...
  │     │
  │     ├── Store / Location  ← a physical outlet  (x-store-id header, replaces x-project-id)
  │     │     │
  │     │     └── POS Terminal  ← the actual tablet device
  │     │           │
  │     │           └── Orders
  │     │
  │     ├── Products / Categories / Customers  (tenant-scoped, shared across stores)
  │     └── Users / Staff
  │
...
1. **`x-tenant-id` header** — read by `TenantMiddleware` on every request (except login). Stored in request-scoped `TenantContextService`.
...
4. **`x-store-id` header** — read by `StoreMiddleware` (replaces `ProjectMiddleware`). Stored in request-scoped `StoreContextService`. Modules that are store-scoped (orders, terminals) opt into this.
...
| `Cashier` | Store/Terminal | Take orders, process payments, create customers |
...
### Store / Location
Replaces "Project" from base-template. A tenant may have one or many physical outlets.
...
### POS Terminal
A physical tablet/device within a store. One store can have multiple terminals (e.g. "Till 1", "Till 2", "Bar").
...
### Stores
```
GET    /stores                  → Store[] for current tenant
POST   /stores                  (owner+)
...
### POS Terminals
```
GET    /stores/:storeId/terminals         → PosTerminal[] for that store
POST   /stores/:storeId/terminals         (manager+) — registers a terminal, returns deviceToken
PATCH  /stores/:storeId/terminals/:id     (manager+)
DELETE /stores/:storeId/terminals/:id     (manager+)
```
...
### Reports
All report endpoints support optional `?storeId=` and `?terminalId=` filters for drill-down.
Add `?eventId=` to scope any report to a specific event (server joins via `TerminalEvent`).

```
GET    /reports/summary         ?from=&to=&storeId=&terminalId=&eventId= → { totalOrders, totalRevenue, totalCustomers, newCustomers, ...deltas }
GET    /reports/sales           ?from=&to=&groupBy=day|week|month&storeId=&eventId= → SalesData[]
GET    /reports/top-products    ?from=&to=&limit=10&storeId=&eventId= → TopProduct[]
GET    /reports/product-status  → { active, inactive, draft }
GET    /reports/recent-orders   ?limit=5&storeId=&terminalId= → Order[]
```
...
**ClaPos Phase 1:**
- [ ] Tenant + Store + POS Terminal setup (registration flow)
...
- [ ] Full reports suite with `storeId` / `terminalId` filters
