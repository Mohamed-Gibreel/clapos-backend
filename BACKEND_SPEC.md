      ...
      "items": [
        {
          ...
          "notes": null,
          "variations": [
            { "groupName": "Size", "optionName": "Large", "priceModifier": 0.5 }
          ]
        }
      ]
    }
...
- [x] `POST /orders/:id/void` + `POST /orders/:id/refund` — proper `Voided`/`Refunded` statuses, `reason`, `refundAmount`; business-rule guards (void: open/in_progress/completed only; refund: completed only, amount ≤ total)
- [x] Tax calculation applied to orders (`subtotal - discount * taxRate`); pulls active `TaxConfig` at checkout
...
- [x] Tax config endpoints (`GET/POST/GET:id/PATCH/DELETE /tax-config`)
- [x] Feature flags endpoint (`GET/POST/GET:id/PATCH/DELETE /feature-flags`)
- [x] Discount/voucher endpoints (`GET/POST/GET:id/PATCH/DELETE /discounts`, `POST /discounts/validate`)
...
- [x] `GET /user/:id` + `PATCH /user/:id` (tenant-scoped; hashes password if updated; validates role)

### Phase 3 — Growth Features (in progress)

- [x] Events + TerminalEvent management endpoints (`POST/GET/GET:id/PATCH/DELETE /events`, `GET/POST /events/:id/terminals`, `DELETE /events/:id/terminals/:terminalId`, `GET /terminals/:id/events`)
- [x] Event performance reporting — `?eventId=` on all reports already works; SuperAdmin cross-tenant view
- [x] Table management endpoints (`POST/GET/GET:id/PATCH/DELETE /tables`, `PATCH /tables/:id/status`)
- [x] Customer membership endpoints (`GET/POST/PATCH /customers/:id/membership`) — creating a membership auto-sets `customer.isMember = true`
- [ ] Real-time table status (WebSocket or SSE) — **next up**
- [ ] Advanced discount types (per-item, member-only)
- [ ] Sales export (PDF/CSV generation via BullMQ)

### Last stopped

Session ended after completing all Phase 3 non-realtime features. Remaining work:

1. **WebSocket / SSE for table status** — when `PATCH /tables/:id/status` fires, push the update to connected POS clients so table layout refreshes in real-time without polling.
2. **Advanced discounts** — per-item discount application; member-only discount flag on `Discount` entity.
3. **Sales export** — `GET /reports/export?from=&to=&format=csv|pdf` endpoint; offload generation to a BullMQ job, return a pre-signed S3 URL when ready.

Latest migration: `1785604863272-migration.ts` (adds `table` and `membership` tables).
