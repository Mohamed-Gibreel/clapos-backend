...
export class TerminalService {
  ...
  async update(id: string, dto: UpdateTerminalDTO) {
    ...
    try {
      ...
      if ((updateRes.affected ?? 0) <= 0) {
        return Result.error({ error: ['Unable to update terminal'], errorCode: HttpStatus.UNPROCESSABLE_ENTITY });
      }
  ...
  async findOne(options: FindOneOptions<PosTerminal>) {
    ...
    try {
      ...
      if (!terminal) {
        return Result.error({ error: ['Terminal not found'], errorCode: HttpStatus.NOT_FOUND });
      }
